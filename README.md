# SharedSII

这里将会收录作者有兴趣整理的SIICS课程笔记。

目前仅收录**最优化理论与算法**一门课程笔记。持续更新中。

课程仓库：[kelvinfkr/algorithm_and_optimization_for_learning](https://github.com/kelvinfkr/algorithm_and_optimization_for_learning)

笔记致敬：[CodingMizore/SharedFDU: course notes for everyone](https://github.com/CodingMizore/SharedFDU)

## 6/8更新

Week14 PPO和RLHF已更新，包含exercise14。

## 5/28更新

Week11 在线优化与最优控制（含习题）已更新，包含exercise11。

Week12 MDP与Tabular RL理论已更新。
